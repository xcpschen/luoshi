...
const adbShell = async (args: string[], deviceId?: string,option?: {}) => {
    const controller = await spawnShell(args, option, deviceId);
    return await controller.result();
};

const disconnectByShell = async (deviceId: string) => {
    return await adbShell(["-s", deviceId, "disconnect"]);
};
...
const forwardPort = async (host: string, port: number,) => {
    return await adbShell(["-s",host+":"+port, "forward", "tcp:"+port, "tcp:"+port]);
};

const removeForwardPort = async (host: string, port: number) => {
    return await adbShell(["-s",host+":"+port, "forward", "--remove", "tcp:"+port]);
};
...
const connectUSB = async (id: string) => {
    await (await getClient()).getDevice(id).usb().connect();
};

const disconnectUSB = async (id: string) => {
    await (await getClient()).getDevice(id).usb().disconnect();
};
...
const filePush = async (
    ...
    return new Promise((resolve, reject) => {
        ...
        transfer.on("end", async () => {
            const res = await adbShell(["-s", id, "shell","am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE -d file://"+devicePath])
            console.log("filePush.res", res)
            resolve({
                localPath,
                devicePath,
            });
