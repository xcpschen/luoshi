...
import DeviceForwardPort from "./DeviceForwardPort.vue";
import DeviceRemoveForwardPort from "./DeviceRemoveForwardPort.vue";
...
const doDelete = async (device: DeviceRecord) => {
    Dialog.confirm(t("device.deleteConfirm")).then(async () => {
        ...
        try {
            if (device.type === EnumDeviceType.USB) {
                await window.$mapi.adb.adbShell(["-s", device.id, "disconnect"]); 
            }
            ...
            <div class="flex-shrink-0 flex flex-col p-0 gap-1 w-9">
                ...
                <a-tooltip :content="$t('device.fileUpload')">
                    <a-button class="ml-1" @click="emit('file-upload', props.record)">
                        <template #icon>
                            <icon-upload class="text-gray-400"/>
                        </template>
                    </a-button>
                </a-tooltip>
