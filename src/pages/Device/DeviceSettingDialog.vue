...
const show = (record: DeviceRecord) => {
    ...
    formData.value.videoBitRate = record.setting?.videoBitRate || "2M";
    formData.value.maxFps = record.setting?.maxFps || "60";
    formData.value.videoCodec = record.setting?.videoCodec || "h265";
    formData.value.videoBuffer = record.setting?.videoBuffer || "50";
    formData.value.maxSize = record.setting?.maxSize || "720";
    visible.value = true;
};
...
const doSubmit = async () => {
    await deviceStore.updateSetting(device.value!.id, {
        ...
        videoCodec: formData.value.videoCodec,
        videoBuffer: formData.value.videoBuffer,
        ...
        <div v-if="visible" class="-mx-5 -my-6">
            <div class="overflow-y-auto p-5" style="height: calc(80vh - 200px)">
                <div>
                    ...
                    <div class="flex mb-3">
                        <div class="flex-grow">{{ $t("device.videoCodec") }}</div>
                        <div class="">
                            <a-input v-model="formData.videoCodec" size="small" :placeholder="$t('device.emptyConfigPlaceholder')" />
                        </div>
                    </div>
                    <div class="flex mb-3">
                        <div class="flex-grow">{{ $t("device.videoBuffer") }}</div>
                        <div class="">
                            <a-input v-model="formData.videoBuffer" size="small" :placeholder="$t('device.emptyConfigPlaceholder')" />
                        </div>
                    </div>
                    <div class="flex mb-3">
                        <div class="flex-grow">{{ $t("device.maxSize") }}</div>
                        <div class="">
                            <a-input v-model="formData.maxSize" size="small" :placeholder="$t('device.emptyConfigPlaceholder')" />
                        </div>
                    </div>
                    <div class="flex mb-3">
                        <div class="flex-grow">
